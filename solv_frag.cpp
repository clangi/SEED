...
float *RePaCh_Fr -------- Modified rec partial charges (for the charged residues 
...
float **RoSFCo ---------- Frag coordinates in the seeded conformation
float **FrCoor ---------- Frag coordinates in the original location
float *FrPaCh ----------- Frag partial charges
...
float **Frdist2 --------- Squared interatomic frag distances
float **dist2 ----------- Squared interatomic rec-frag distances
float *FrMinC_orig ------ Min (along x,y,z) of FrCoor
float *FrMaxC_orig ------ Max (along x,y,z) of FrCoor
...
######################################################################*/
{
  ...
  float **ExRoSFCo,**dist,FrMinC[4],FrMaxC[4];
...
float **ExRoSFCo -------- Coor of the frag and of those rec atoms that are at a 
                          distance < 2*WaMoRa from at least 1 frag atom
float **dist ------------ Frag-rec interatomic distances
float FrMinC[4] --------- Min (along x,y,z) of RoSFCo
float FrMaxC[4] --------- Max (along x,y,z) of RoSFCo
...
/* Make some new arrays for the "Extended fragment" (= the fragment plus 
  ...
  ExRoSFCo = matrix(1,ExFrAtNu,1,3);
...
/* Calculate the electrostatic intra-ligand interactions
  ...
  *PFrDesoElec *= corr_fr_deso;
  ...
  free_matrix(dist,1,FrAtNu,1,ReAtNu);
  ...
  free_matrix(ExRoSFCo,1,ExFrAtNu,1,3);
    ...
    for (jat=1;jat<=FrAtNu;jat++) {
      if (FrPaCh[jat] != 0. ) {
        if (dist2[jat][NeighList3[iat]] > 0.) {
          ...
          Rij = (float) ReEffRad[NeighList3[iat]] * FrEffRad[jat];
...
float **ReCoor ---------- Rec Coordinates
float *FrPaCh ----------- Rec partial charges
